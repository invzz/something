...
class skyBoxService
{
  ...
  Shader            *shader;
  bool               useHDR = false;
  skyBoxServiceLocs *locs;
  TextureCubemap     cubemap = {0};

  public:
  skyBoxService(char *filename, Shader *shader) : skyboxFileName(filename), shader(shader), locs(new skyBoxServiceLocs(shader)) { init(); };
  void init() { locs->init(shader); }

  void setHDR(int *useHDR) { SetShaderValue(*shader, locs->doGamma, useHDR, SHADER_UNIFORM_INT); }
  void setEnvironmentMap(int *cubemap) { SetShaderValue(*shader, locs->environmentMap, cubemap, SHADER_UNIFORM_INT); }

  void SetSkyBox(Model *s)
  {
    s->materials[0].shader = *shader;
