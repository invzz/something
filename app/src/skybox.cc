...
int main(void)
{
  ...
  SetConfigFlags( FLAG_FULLSCREEN_MODE); // Try to setup VSYNC on GPU
  ...
  skybox.materials[0].shader =
    LoadShader(TextFormat(SHADER_DIR "skybox.vs"), TextFormat(SHADER_DIR "skybox.fs"));

  SetShaderValue(skybox.materials[0].shader, GetShaderLocation(skybox.materials[0].shader, "environmentMap"), (int[1]){MATERIAL_MAP_CUBEMAP},
                 SHADER_UNIFORM_INT);
  SetShaderValue(skybox.materials[0].shader, GetShaderLocation(skybox.materials[0].shader, "doGamma"), (int[1]){useHDR ? 1 : 0}, SHADER_UNIFORM_INT);
  SetShaderValue(skybox.materials[0].shader, GetShaderLocation(skybox.materials[0].shader, "vflipped"), (int[1]){useHDR ? 1 : 0}, SHADER_UNIFORM_INT);

  // Load cubemap shader and setup required shader locations
  Shader shdrCubemap = LoadShader(TextFormat(SHADER_DIR "cubemap.vs"), TextFormat(SHADER_DIR "cubemap.fs"));
  ...
  if(useHDR)
    ...
    {
      Image img                                              = LoadImage(SKYBOX_DIR "skybox.png");
